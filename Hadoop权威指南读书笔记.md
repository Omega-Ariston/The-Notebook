...
### 第十一章 管理Hadoop
- namenode目录下会有一个VERSION文件
- VERSION中的namespaceID：文件系统命名空间唯一标识符，namenode首次格式化时创建，对于联邦HDFS非常重要
- namenode目录下的in_use.lock文件是一个锁文件，namenode用其给存储目录加锁，可以避免其它namenode实例同时使用同一个存储目录
- 文件系统执行写操作时（如创建或移动文件），这些事务首先被记录到编辑日志中。namenode在内存中维护文件记录的元数据，编辑日志被修改时相关元数据信息被同步更新。内存中的元数据可以支持客户端的读请求
- 编辑日志在磁盘上有多个segment文件，名称由前缀edits及后缀组成，后缀用于指示该文件包含的事务ID
- 任意时刻只有一个segment处于可打开状态（前缀包含inprogress）
- 每个事务完成后，向客户端发送成功代码前，文件都需要更新与同步
- 每个fsimage文件都是系统元数据的一个完整的永久性检查点（前缀中是映像文件中的最后一个事务），包含文件系统中所有目录和文件inode的序列化信息，但不维护datanode的块映射关系，因为这些信息都放内存里，datanode加入集群时namenode会向datanode索取块列表以建立映射关系，并且还会定期征询最新信息
- fsimage是一个大型文件（可高达几G），频繁执行写操作会使系统运行缓慢
- namenode发生故障时会将最近的fsimage载入内存以重构元数据状态，并从相关点开始向前（？）执行编辑日志中的每个事务
- 辅助namenode会定期用http get和put来获取主namenode上的映像和日志，并在合并完之后传回给主namenode。创建检查点的条件受两个参数控制，一是间隔时间，二是事务数量
- 与namenode不同，datanode的存储目录是初始阶段自动创建的，不需要额外格式化
- HDFS数据块存储在以blk_为前缀名的文件中，文件名包含了该文件存储的块的原始字节数，每个块有对应的.meta元数据文件，包含了头部（版本与类型信息）和该块各区段的校验和
- 每个块属于一个数据块池，每个数据块池有自己的存储目录，目录根据块池ID形成（与namenode的VERSION文件中数据块池ID相同）
- 一个目录中的数据块上限由参数配置，超出后会创建当前目录的子目录，以避免同一级目录下存放过多文件
- 如果dfs.datanode.data.dir属性指定了不同磁盘上的多个目录，数据块会以轮询方式写到各个目录中
- 同一个datanode上不会出现重复磁盘块，不同datanode之间才有可能有
- 安全模式：namenode启动时，会先将映像文件载入内存并执行编辑日志中的各项操作，成功建立文件系统元数据映像后会生成新的fsimage文件和一个空白编辑日志。这各个过程中namenode运行在安全模式下，对客户端而言是只读的
- 安全模式下每个datanode会向namenode发送最新的块列表信息以使其存在内存中。当满足最小副本条件（整个文件系统中指定百分比数量的块满足最少复制数）时namenode会在30秒后退出安全模式
- 管理员可以随时让namenode进入安全模式
- dfsadmin工具可以用于查找HDFS状态信息以及在HDFS上执行管理操作
- fsck工具可以用于检查HDFS中文件的健康状况，比如查找在所有datanode中均缺失的块以及过少或过多副本的块，也可以用来查找指定文件的数据块位置
- 这一章的结尾详细说明了往集群中增/删datanode节点以及升级Hadoop的步骤