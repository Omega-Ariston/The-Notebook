...
- 大对象是垃圾收集过程中的灾难，需要分配的空间大，做对象复制时的开销也大。所以可以用参数-XX：PretenureSizeThreshold设置大于此阈值的对象直接进入老年代（只对Serial和ParNew生效）。
- 对象头中记录的年龄数会随着Minor GC的次数而增加，当增加到15岁时会被加入老年代，老年岁数可通过-XX：MaxTenuringThreshold参数设置
- 如果Surviro空间中相同年龄的所有对象大小总和大于一半空间，则年龄大于等于这些对象的对象就直接进入老年代
