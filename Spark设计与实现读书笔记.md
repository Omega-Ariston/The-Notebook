...
#### 7.2.1 哪些数据需要被缓存
- cache()操作表示将数据直接写入内存。
- cache是lazy操作，不会立即执行，当执行到cache()时只会对需要缓存的对象标记，直到使用该缓存的job运行时再将需要缓存的对象写入内存。
- cache操作只是将数据缓存到内存中，如果需要将数据缓存到内存和磁盘中，可以使用persist(MEMORY_AND_DISK)，后续会介绍。
- 当缓存对象很大时（如包含上亿个record），存储缓存会消耗大量空间，这时需要权衡计算代价和存储代价，比如不缓存重复数据，而是每次通过从原始数据中计算得到。
- 总的来说，缓存是以空间换时间，一般当数据满足以下3条，就可以进行缓存：
    1. 会被重复使用的数据，确切地说是会被多个job共享使用的数据，对于迭代型和交互型应用非常适合。
    2. 数据不宜过大，虽然可以存放到磁盘中，但IO的代价会变大，有时还不如重新算快。
    3. 非重复缓存的数据。也就是说如果缓存了某个RDD，那么该RDD通过一对一窄依赖连接的parent RDD就不需要被缓存了。
- 除了RDD，广播数据和task计算结果数据也可以被缓存，以及面向结构化的数据结构DataSet、DataFrame与RDD一样也可以被缓存。
#### 7.2.2 包含数据缓存操作的逻辑处理流程和物理执行计划
- Spark会先假设应用没有数据缓存，按照第3章的规则正常生成逻辑处理流程（RDD之间的数据依赖关系），然后从第2个job开始，将cached RDD之前的RDD都去掉，得到削减后的逻辑处理流程。最后按照第4章给出的正常规则将逻辑处理流程转化为物理执行计划。
#### 7.2.3 缓存级别
- Spark从3个方面考虑了缓存级别（Storage_Level）。
    1. 存储位置。可以将数据缓存到内存和磁盘中，内存空间小但读写速度快，磁盘则反之。
    2. 是否序列化存储。如果对数据进行序列化，则可以减少存储空间，方便网络传输，但计算时需要进行反序列化，增加计算时延。
    3. 是否将缓存数据进行备份。将缓存数据复制多份并分配到多个节点，可以应对节点失效带来的缓存数据丢失问题，但需要更多的存储空间。
- Spark将缓存级别分为12类（位置、序列化与否、备份与否的不同组合），用户可以使用rdd.persist(Storage_Level)对RDD按不同缓存级别进行缓存。
- 缓存级别针对的是RDD中的全部分区，对RDD中每个分区中的数据都会进行缓存。
- **对于MEMORY_ONLY级别来说，只使用内存进行缓存。如果某个分区在内存中放不下，就不对该分区进行缓存。后续job中的task计算需要使用这个分区中的数据时，需要重新计算得到该分区。**
- 因为存储到磁盘前需要对数据进行序列化，所以DISK_ONLY级别也需要序列化存储。
- 选择缓存级别时考虑的两个问题：
    1. 是否有足够内存、磁盘空间进行缓存。
    2. 如果数据缓存到磁盘上，读取数据的时间是否大于重新计算出该数据的时间。
#### 7.2.4 缓存数据的写入方法
- 以mappedRDD为例，当map每计算出一个record后，就会执行persist操作将该record写入磁盘，再执行下一步combine操作，而不是先进行combine再进行persist。因为record被计算出来后会放入HashMap结构中进行combine聚合，聚合后，计算出来的record就会被清除。
- 缓存写入的实现细节：Spark在每个Executor进程中分配一个区域，以进行数据缓存，该区域由BlockManager来管理。在计算出一个分区的数据后，会将数据存放到BlockManager中的memoryStore内。memoryStore包含了一个LinkedHashMap，用来存储RDD的分区。该LinkedHashMap中的Key是blockId，即rddId+partitionId，如rdd_1_1，Value是分区中的数据，LinkedHashMap基于双向链表实现。
#### 7.2.5 缓存数据的读取方法
- 当某个RDD被缓存后，该RDD的分区变成CachedPartitions。
- 可以通过使用到缓存的RDD的.toDebugString()方法来查看使用到的缓存的CachedPartitions的存储位置及占用的空间大小。
- 当job需要读取某个缓存RDD中的分区时，会先去本地的BlockManager中查找该分区是否被缓存，如果在，则直接从memotyStore读取，否则需要通过getRemote()进行远程读取，这个过程会涉及序列化和反序列化，并且是一条一条record进行读取。
#### 7.2.6 用户接口的设计
- cache()实际上等同于persist(MEMORY_ONLY)
- 用户可以对缓存RDD进行unpersist()操作实现缓存数据回收
- persist和unpersist操作只能针对在程序中可见的RDD进行，在执行过程中由Spark自动生成的RDD不能被用户操作。
#### 7.2.7 缓存数据的替换与回收方法
1. 自动缓存替换
    - 缓存替换指的是当需要缓存的RDD大于当前可利用的空间时，使用新的RDD替换旧的RDD（可能有多个），这个过程对用户无感。
    1. 选择哪些RDD进行替换：直观上说，如果旧的RDD需要被再次利用，就不应该被替换。但是Spark采用的是动态生成job的方式，在执行action操作时才生成job，并且在执行过程中Spark只知道cached RDD是否会被当前job用到，而无法预知其是否会被后续的job用到。因此Spark采用了LRU替换算法，优先替换掉当前最长时间没有被使用过的RDD，**这种方式有可能替换掉后续还会被使用的RDD。**
    2. 需要替换多少个旧RDD才开始存储新的RDD：如同前面篇幅所述，对于需要缓存的RDD，Spark会在其计算的过程中就进行缓存，而且是每计算一个record就进行存储，因此在缓存结束前Spark不能预知该RDD需要的存储空间。因此Spark采用动态替换策略，在当前可用内存空间不足时，能完LRU替换一个或多个RDD（具体数目与一个动态阈值相关），然后开始存储新的RDD，依此类推直到存放完新的RDD。如果替换掉所有旧的RDD都存不下新的RDD，那么会根据存储级别选择是将新的RDD存放到磁盘还是直接不存了。
2. Spark LRU算法的实现及讨论
    - Spark使用了LinkedHashMap自带的LRU功能实现缓存替换。
    - 在进行缓存替换时，RDD的分区数据不能被同一个RDD的其它分区数据替换。
3. 用户主动回收缓存数据
    - 从前文所述可以得知Spark难以获取cached RDD的生命周期，因此难以精确、智能地进行缓存替换。
    - Spark允许用户通过unpersist操作自己设置进行回收的RDD和回收的时间。
    - 不同于persist操作的lazy实现，unpersist操作是立即生效的。
    - 用户可以设定unpersist操作的blocking参数设置该操作是同步阻塞的还是异步执行。Spark默认是同步执行，需要等unpersist结束后再进行下一步操作。
    - unpersist和persist执行方式的区别会导致如果unpersist语句设置的位置不当，会造成与用户预期效果不一致的结果。比如在action操作前就进行了unpersist，会导致缓存被清除。
### 7.3 与Hadoop MapReduce的缓存机制进行对比
- 对比Hadoop MapReduce，Spark的缓存机制是Spark的优势之一。
- Hadoop MapReduce虽然设计了DistributedCache分布式缓存，但并不是用于存放job的中间结果，而是用于缓存job运行所需文件的，比如所需的jar文件、每个map task需要读取的辅助文件（如一部词典）、一些文本文件等。
